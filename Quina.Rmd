...
fn <- "data/D_QUINA.HTM"
if (file.exists(fn)) {
  if (difftime(Sys.time(),file.info(fz)$ctime, units="days") > 2) {
    file.remove(fz)
    file.remove(fn)
...
for(i in 2:length(thetable$children))
{
     ...
     vConcurso <- c(vConcurso, suppressWarnings(as.numeric(thetable$children[[i]]$children$td$children$text$value)))
     vDate <- c(vDate, as.Date(thetable$children[[i]]$children[[2]]$children$text$value, "%d/%m/%Y"))
     vDezena1 <- c(vDezena1, suppressWarnings(as.numeric(thetable$children[[i]]$children[[3]]$children$text$value)))
     vDezena2 <- c(vDezena2, suppressWarnings(as.numeric(thetable$children[[i]]$children[[4]]$children$text$value)))
     vDezena3 <- c(vDezena3, suppressWarnings(as.numeric(thetable$children[[i]]$children[[5]]$children$text$value)))
     vDezena4 <- c(vDezena4, suppressWarnings(as.numeric(thetable$children[[i]]$children[[6]]$children$text$value)))
     vDezena5 <- c(vDezena5, suppressWarnings(as.numeric(thetable$children[[i]]$children[[7]]$children$text$value)))
}
...
```{r} 
dfQuinaFreq <- as.data.frame(table(c(tblQuina$d01, tblQuina$d02, tblQuina$d03, tblQuina$d04, tblQuina$d05)))
tblQuinaFreq <- tbl_df(dfQuinaFreq)
arrange(tblQuinaFreq, desc(Freq))
```

4. Checking the results according to a certain `concurso`.
```{r}
arg1 <- 3712 
